...
uint64_t read_bits(bit_reader* reader, size_t bitsToRead)
{
	...
	if (reader->dataSizeBytes < BIT_READER_RAW_READ_SIZE)
	{
	...
	if (read_is_within_bounds(reader, bitsToRead))
	{
		...
		if (reader->cursor + BIT_READER_RAW_READ_SIZE > reader->dataSizeBytes)
		{
			size_t safeStartBit = (reader->dataSizeBytes - BIT_READER_RAW_READ_SIZE) * 8;
