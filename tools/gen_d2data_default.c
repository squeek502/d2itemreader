...
{
	d2err err;
	err = d2data_load_armors_from_file("data/Armor.txt", &g_d2data);
	if (err != D2ERR_OK)
	{
		printf("failed to load data/Armor.txt: %s\n", d2err_str(err));
		return 1;
	}
	err = d2data_load_weapons_from_file("data/Weapons.txt", &g_d2data);
	if (err != D2ERR_OK)
	{
		printf("failed to load data/Weapons.txt: %s\n", d2err_str(err));
		return 1;
	}
	err = d2data_load_miscs_from_file("data/Misc.txt", &g_d2data);
	if (err != D2ERR_OK)
	{
		printf("failed to load data/Misc.txt: %s\n", d2err_str(err));
		return 1;
	}
	err = d2data_load_itemstats_from_file("data/ItemStatCost.txt", &g_d2data);
	if (err != D2ERR_OK)
	{
	...
	pos[0] = 0; pos[1] = strset_count(g_d2data.stackablesSet);
	strset_iterate(g_d2data.stackablesSet, &dumpstrset, pos);
	printf("};\n");

	printf("const char* d2data_default_weapons[] = {");
	pos[0] = 0; pos[1] = strset_count(g_d2data.weaponsSet);
	strset_iterate(g_d2data.weaponsSet, &dumpstrset, pos);
	printf("};\n");

	printf("const char* d2data_default_armors[] = {");
	pos[0] = 0; pos[1] = strset_count(g_d2data.armorsSet);
	strset_iterate(g_d2data.armorsSet, &dumpstrset, pos);
	printf("};\n");
	...
	for (int id = 0; id < D2DATA_MAX_ITEMSTATCOST_IDS; id++)
	{
		...
		if (id == stat->id)
		{
			int isLast = (id == D2DATA_MAX_ITEMSTATCOST_IDS - 1) || (g_d2data.itemstats[id + 1].id != id + 1);
